...
- [ ] Cobertura sobre `storage/`, `embedding/`, `domain/`; `bun run check` limpo; README de uso (init → `claude mcp add cortex -- cortex serve --mcp` → fluxo básico). Remover `scripts/smoke-*.ts` (contexto já absorvido pela 1.5/2.2).
